...
<h4 class="mb-3 mt-4">Approval Pemakaian Spare Barang</h4>

@forelse($spareList as $spare)
    <div class="card shadow-sm mb-4">
        <div class="card-body">
            <div class="d-flex align-items-center mb-3">
                <div class="rounded-circle bg-warning text-dark d-flex align-items-center justify-content-center"
                    style="width:48px;height:48px;font-size:1.2rem;">
                    {{ strtoupper(substr($spare['nama'], 0, 1)) }}
                </div>
                <div class="ms-3">
                    <div class="fw-bold fs-5">{{ $spare['nama'] }}</div>
                    <div class="text-muted small">SR {{ $spare['no_sr'] }}</div>
                </div>
            </div>
            ...
            <div class="border-start border-4 border-warning ps-3 py-2 mb-2 bg-light">
                <div class="fw-semibold">{{ $spare['item_lbl'] }}</div>
                <div class="small">
                    QTY DIPAKAI: <span class="badge bg-secondary">{{ $spare['pemakaian']->qty }}</span>
                    <span class="ms-2">SISA SPARE: <span class="badge bg-success">{{ $spare['sisa'] }}</span></span>
                </div>
                @if($spare['pemakaian']->catatan)
                    <div class="small text-muted mt-1">Catatan: {{ $spare['pemakaian']->catatan }}</div>
                @endif
            </div>
            ...
            <div class="card mt-3">
                <div class="card-body">
                    ...
                    <div class="row g-2">
                        <div class="col-md-6">
                            <form action="{{ route('gudang.approval-demob.spare.approve', [$idgudang, $spare['pemakaian']->id]) }}"
                        ...
                        <div class="col-md-6">
                            <form action="{{ route('gudang.approval-demob.spare.reject', [$idgudang, $spare['pemakaian']->id]) }}"
                                method="POST" class="form-approval"
                                onsubmit="return confirm('Tolak pemakaian spare ini?')">
                                @csrf
                                <input type="hidden" name="catatan" class="catatan-mirror">
                                <button type="submit" class="btn btn-outline-danger w-100">
                                    <i class="fas fa-times me-1"></i> Not Approve
...
@empty
    <div class="card shadow-sm">
        <div class="card-body text-center text-muted py-5">
            <i class="fas fa-box-open fa-2x mb-3 opacity-25 d-block"></i>
            Tidak ada pemakaian spare barang yang menunggu approval.
        </div>
    </div>
@endforelse
